...
def manual_entry():
    ...
    with st.form("json_form", clear_on_submit=False):
        ...
        transaction_id = st.text_input(
            "Transaction ID", key="transaction_id", value=st.session_state.get("transaction_id", ""))
        date = st.date_input("Date (YYYY-MM-DD)", key="date",
                             value=st.session_state.get("date", None))

        time = st.number_input("Time (HH.MM)", min_value=0.0, max_value=24.0, format="%.2f",
                               step=0.01, key="time", value=st.session_state.get("time", None))

        st.subheader("Items")
        if "component_count" not in st.session_state:
            st.session_state.component_count = len(
                st.session_state.get("items", [])) + 1
            ...
            with st.expander(f"Item {i + 1}", expanded=True):
                ...
                with cols[0]:
                    item_name = st.text_input(
                        f"Item Name {i + 1}", key=f"text_field1_{i}", value=st.session_state.get(f"text_field1_{i}", ""))
                with cols[1]:
                    total_price = st.number_input(f"Total Price {i + 1}", min_value=0.0, format="%.2f",
                                                  step=0.1, key=f"text_field2_{i}", value=st.session_state.get(f"text_field2_{i}", None))
                with cols[2]:
        ...
        if submitted:
            ...
            updated_json_data = DisplayDetails.print_response_from_manual_entry(
                json_data)
